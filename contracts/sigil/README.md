...
Mint a deterministic on-chain SVG avatar (a "sigil") for a derived account address.

- One sigil per derived address
- Optional on-chain `tag` stored alongside the sigil
- No external assets, no backend
...
- Deterministic avatars: one derived address, one SVG.
...
- On-chain PFPs that actually live on-chain (revolutionary concept, I know)
